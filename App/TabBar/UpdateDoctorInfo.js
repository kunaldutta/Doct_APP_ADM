...
const UpdateDoctorInfo = ({ navigation, route }) => {
  const { clinic_id } = route.params;
  const { doctorData } = route.params;
  console.log('doctorData',doctorData);
  const [doctorName, setDoctorName] = useState(doctorData?.doctor_name || '');
  const [doctorExp, setDoctorExp] = useState(doctorData?.doctor_exp || '');
  const [doctorSpec, setDoctorSpec] = useState(doctorData?.doctor_spec || '');
  const [doctorDegree, setDoctorDegree] = useState(doctorData?.degree || '');
  const [doctorImage, setDoctorImage] = useState({uri: doctorData?.doctor_image} || null);
  const [allFieldError, setAllFieldError] = useState(false);
  const [allFieldValid, setAllFieldValis] = useState(false);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.post('https://developersdumka.in/ourmarket/update_doctor_info.php', formData, {
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ left: '1%', width: '80%', alignItems: 'center' }}>
          <Text style={{ fontSize: 20, fontWeight: 'bold' }}>Update Doctor Detail</Text>
        </View>
      </View>
